...
import { IncreaseFactor, SkillEntry } from '../../../../models/base-creation.model';
import { ATTR_COLORS } from '../../../../constants/attribute-colors.const';
import { ATTR_KEY } from '../../../../utils/derived-stats.util';
import { computeRoutine } from '../../../../utils/utils';
...
@Component({
  ...
  imports: [FormsModule, AccordionModule, InputNumber, InputTextModule, SelectModule, TableModule, TooltipModule, D20RollButtonComponent],
...
export class TalentsComponent {
  ...
  readonly categories = computed(() => {
    ...
    return CATEGORY_DEFS.map((cat) => ({
      ...
      rows: cat.talents.map((def) => ({
        def,
        fw: (skills?.[cat.key] as SkillEntry[] | undefined)?.find((s) => s.name === def.name)?.fw ?? 0,
      })),
    }));
  });

  /**
   * Routine value (DSA5 + optional attribute rule), computed from the talent's FW and the character's
   * check-attribute values — no longer a manual field. Shows the hardest probe modifier still routinable.
   */
  getRoutine(key: SkillKey, def: TalentDefinition): string {
    const c = this.character();
    const fw = (c?.skills[key] as SkillEntry[] | undefined)?.find((s) => s.name === def.name)?.fw ?? 0;
    const attrs = c?.attributes;
    if (!attrs) return '-';
    return computeRoutine(fw, def.check.map((code) => attrs[ATTR_KEY[code as keyof typeof ATTR_KEY]]));
  }
  ...
  updateFw(key: SkillKey, def: TalentDefinition, value: number | null): void {
    ...
    this.character.update((c) => {
      if (!c) return c;
      const cat = [...(c.skills[key] as SkillEntry[])];
      const idx = cat.findIndex((s) => s.name === def.name);
      if (idx >= 0) {
        cat[idx] = { ...cat[idx], fw: value };
      } else {
        cat.push({ name: def.name, fw: value });
      }
